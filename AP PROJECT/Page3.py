...
def show():
    ...
    c.execute('''Select o.Opt_name, b.type, r.Seat_Availavle, b.fare from bus as b, operator as o, run as r, route as re where re.station_name = ? and  b.operator_id = o.operator_id and b.bus_id = r.bus_id''',(ent_From.get(),))
    records = c. fetchone()
    print(records)
    count =1
    for record in records:
        Label(root, text = str(record)).grid(row=9, column=count)
        count = count+1
...
house = PhotoImage(file = "C:\\Users\\hp\\Documents\\AP PROJECT\\home.png")
home_button = Button(root, image = house)
